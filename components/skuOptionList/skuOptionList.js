...
const { sku: Sku } = require("peanut-all");
	...
	methods: {
		...
		setOptionList() {
			...
			try {
				...
				// SKU数组 不过滤库存为0的
				let currentSku = properties.map(({ name }) => ({
					key: name,
					value: "",
				}));

				// 所有规格/增值规格数组
				let currentSpecial = special_attributes.map(({ key }) => ({
					key,
					value: "",
				}));
				let currentRelation = related_product.map(({ key }) => ({
					key,
					value: "",
				}));
		...
		onSelectOption(e) {
			...
			let dataArr = {
					...
					special_attributes: "currentSpecial",
					related_product: "currentRelation",
				},
			...
			// 选中/取消
			propValue =
				this.data[dataKey][nameIndex].value === propValue ? "" : propValue;
