...
import { updateTabbar, autoNavigate, getUserProfile, go, debounce } from 'utils/util';
...
Page({
    ...
    onUpdateQuantity: debounce(async function(e) {
        console.log('e', e);
        let { detail, currentTarget: { dataset: { cartId, id }}} = e,
        ...
        try {
            await api.hei.pvmUpdateCart(requestData);
    ...
    async onDelete(e) {
        ...
        if (confirm) {
            ...
            const data = await api.hei.pvmRemoveCart(requestData);
            wx.showToast({
                title: '成功删除',
            });
    ...
    async onClearCart() {
        ...
        if (confirm) {
            const data = await api.hei.pvmClearCart({
                shipping_type
            });
            wx.showToast({
                title: '成功清空购物车',
