!(function () {
    ...
    function badruns(length) {
        ...
        for (i = 3; i < length - 1; i += 2) { if (rlens[i - 2] == rlens[i + 2]
        && rlens[i + 2] == rlens[i - 1]
        && rlens[i - 1] == rlens[i + 1]
        && rlens[i - 1] * 3 == rlens[i]
    ...
    function badcheck() {
        ...
        for (y = 0; y < width - 1; y++) { for (x = 0; x < width - 1; x++) { if ((qrframe[x + width * y] && qrframe[(x + 1) + width * y]
    ...
    function genframe(instring) {
        ...
        for (i = 0; i < eccblkwid; i++) {
            genpoly[i + 1] = 1;
            for (j = i; j > 0; j--) { genpoly[j] = genpoly[j]
