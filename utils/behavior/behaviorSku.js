let pageShare = require("../pageShare");
...
module.exports = Behavior({
	...
	methods: {
		// 商品规格回调
		onOptionSelect(e) {
			let { currentSku, selectedSku, skuMap, currentSpecial, currentRelation } =
					e.detail,
				{
					product,
					product: { related_product, miaosha_enable },
				} = this.data;

			let price = Number(selectedSku.price || product.price) * 100;
			if (miaosha_enable) {
				let { miaosha_end_timestamp, miaosha_start_timestamp, miaosha_price } =
					product;
