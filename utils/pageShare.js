...
import { showModal, showToast } from "./wxp";
import { auth, subscribeMessage, joinUrl } from "./util";
...
export const onDefaultShareAppTimeline = function () {
	...
	const shareMsg = makeShareParams.call(
		this,
		arguments[0],
		arguments[1],
		arguments[2],
...
export const createCurrentOrder = (e) => {
	...
	try {
		...
		if (selectedSku.id) {
			...
			const selectedSkuImage = properties
				? sku_images[firstSelectedSkuPropValue] &&
				  sku_images[firstSelectedSkuPropValue].thumbnail
				: null;
		...
		// 增值规格价格
		item.price = related_posts.reduce(
			(acc, { price }) => acc + price,
			item.price
		);
		order.items = [item];
...
export const wxPay = async (options = {}, order_no, subKeys = []) => {
	try {
		...
		if (order_no) {
			api.hei.orderQuery({ order_no }).then((res) => {
