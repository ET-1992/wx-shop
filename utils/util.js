  ...
  return new Promise((resolve, reject) => {
    ...
    if (isExpired) {
      wx.getUserProfile({
        ...
        success: async (res) => {
          try {
            const { user } = await api.hei.getUserProfile({
              user: res.userInfo,
            });
            wxStorage.set({
              key: USER_KEY,
...
export function getAgainTokenForInvalid() {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      console.log("获取token");
      const { user, access_token, expired_in } = await api.hei.silentLogin({
        code,
      });
...
export function getNodeInfo(id, obj = {}, isComponent, ctx) {
  return new Promise((resolve, reject) => {
    ...
    const query = isComponent
      ? wx.createSelectorQuery().in(ctx)
      : wx.createSelectorQuery();
    query
      .select(`#${id}`)
      .fields(defaultObj, (res) => {
        resolve(res);
      })
      .exec();
  });
...
export function updateTabbar({
  tabbarStyleDisable = false,
  tabbarCartNumDisable = false,
  pageKey = "",
}) {
  const app = getApp();
...
export function textToValue(array = [], text = "") {
  ...
  return (
    (filterValue && filterValue[0] && filterValue[0].value) || array[0].value
  );
}
...
export function valueToText(array = [], value = null) {
  ...
  return (
    (filterValue && filterValue[0] && filterValue[0].text) || array[0].text
  );
}
      ...
      try {
        ...
        ctx.setData({
          authModal: {
            ...
            buttonData: {
              opentype:
                scope === "scope.userInfo" ? "getUserInfo" : "openSetting",
            },
            confirmText: "确定",
            cancelText: "取消",
          },
...
export function checkBeyondDistance(current, target, limitDistance) {
  ...
  console.log(
    `实际距离${Number(distance)}km，限制距离${Number(limitDistance)}km`
  );
  if (!inRange) {
...
export async function subscribeMessage(keys = []) {
  ...
  const tmplIds = subscribeMessageTemplates
    .filter((item) => {
  ...
  try {
    ...
    const isSubscribeMessageTemplates = subscribeMessageTemplates.filter(
      (item) => {
